...
When you need me to pick between concrete choices to proceed, prefer the agent's structured question UI when it is available — it's easier to answer than prose.

When options don't fit that UI (too many choices, long or open-ended directions, code blocks, or a recommendation needs extra context rather than just a pick), present them as a labeled list so I can reference them in my reply:
